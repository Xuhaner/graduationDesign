...
public class ObjectBeanDaoImpl implements ObjectBeanDao{
    // 新建问卷
    ...
    public List ListObjectBean() {
        ...
        try {
            String sql = "select oid,title,createtime,state,team,author from wj_object where team is null order by oid desc";
            con=dbcon.getConnection();
            stm=con.createStatement();
            rs = stm.executeQuery(sql);
            while(rs.next()){
