...
基于web端设计，以JSP核心设计。使用JavaBean + SpringMVC架构进行编写。通过M（model）V（view）C（control）三层来实现系统的功能。意义在于，用户的输入与输出均为JSP页面，本系统采用MVC的设计模式，用户输入输出界面均为JSP页面，即展示层（VIEW），用户的请求（REQUEST）将会被发送到控制层（CONTROL）的一个SERVLET集中处理，根据请求调用模型层（MODEL）的JAVABEAN对请求进行实际处理，并把结果返回给控制层，控制层根据请求的执行结果将响应（RESPONSE）派发到相应的JSP页面。 
...
目前商场上主要的几大问卷系统是腾讯问卷，腾讯问卷不管是界面还是功能，无疑是国内所有调查问卷平台最好用最全面的。它是一个免费、专业的问卷调查系统。提供多种方式创建问卷，简单高效的编辑方式，强大的逻辑设置功能，专业的数据统计和样本甄别，让您轻松开启调研工作。
...
JavaScript，JavaScript一种直译式脚本语言，是一种动态类型、弱类型、基于原型的语言，内置支持类型。它的解释器被称为JavaScript引擎，为浏览器的一部分，广泛用于客户端的脚本语言，最早是在[HTML](https://baike.baidu.com/item/HTML)（标准通用标记语言下的一个应用）网页上使用，用来给HTML网页增加动态功能。
