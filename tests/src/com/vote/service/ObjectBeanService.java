...
public class ObjectBeanService {
	...
	public static int intsertObjectBean(ObjectBean bean) throws Exception {
		...
		try {
			...
			sql = "insert into wj_object(title,discribe,state,remark,anonymousFlag,team,author) values(?,?,?,?,?,?,?)";
			...
			stmt.setString(6, bean.getTeam());
			stmt.setString(7, bean.getAuthor());
	...
	public static int updateObjectBean(ObjectBean bean) {
		...
		String sql1 = "update wj_object set title = '" + bean.getTitle()
				+ "',discribe = '" + bean.getDiscribe() + "',anonymousFlag = '" + bean.getAnonymousFlag() + "'" + ",team ='" + bean.getTeam() + "'" + 
	...
	public static List ListObjectBean() {
		...
		try {
			String sql = "select oid,title,createtime,state,team,author from wj_object where team is null or team = '" + "' order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
	...
	//	 查看问卷;
	public static List ListTeamObjectBean(String teamname) {
		DBConnection dbcon = new DBConnection();
		Connection con = null;
		Statement stm = null;
		ResultSet rs = null;
		List objList=new LinkedList();
		try {
			String sql = "select oid,title,createtime,state,team,author from wj_object where team = '" + teamname + "'  order by oid desc";
			con=dbcon.getConnection();
			stm=con.createStatement();
			rs = stm.executeQuery(sql);
			while(rs.next()){
	...
	public static ObjectBean findObjectBeanByID(int ID) {
		...
		try {
			String sql = "select oid,title,discribe,state,anonymousFlag,team,author from wj_object where oid='" + ID
			+ "'";
			con=dbcon.getConnection();
			stm=con.createStatement();
			 rs = stm.executeQuery(sql);
